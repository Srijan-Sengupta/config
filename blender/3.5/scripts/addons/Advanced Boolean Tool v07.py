...
def auto_UV():
    ...
    if current_mode == ('OBJECT'): 
        ...
        bpy.ops.uv.smart_project(angle_limit=89, island_margin=0.01, scale_to_bounds=False)
        ...
        for mods in obj_mods:
            try:
                if search("_B",mods.name):
                    bpy.ops.object.modifier_apply(  modifier=(mods.name))
            except RuntimeError:
...
def weighted_normal():
    ...
    for obj_n in selected_obj:
        ...
        weighted_nrm_mod.use_face_influence = True
...
def register_key_8155C():
    ...
    if kc:
        ...
        kmi = km.keymap_items.new("wm.call_menu_pie",
                                    ...
                                    alt=True,
                                    shift=False)
        kmi.properties.name = "ABT_OT_menu"
